...

{
  ...
  # Only until `passwd goose` sets a real one; initialPassword applies only
  # while the account has no password set.
  modules.users.initialPassword = "changeme";

  # Easy shell access from the host (`ssh goose@<vm-ip>`) — sidesteps the
  # Boxes TTY-switch problem entirely.
  # NOTE: hardening disables password auth, so provision an SSH key first
  # (`ssh-copy-id goose@<vm-ip>` after setting the password), or temporarily
